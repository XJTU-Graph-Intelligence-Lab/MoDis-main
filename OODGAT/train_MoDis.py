...
# pyg imports
from torch_geometric.datasets import Planetoid, WikiCS, Amazon, Coauthor, LastFMAsia, NELL, CitationFull, \
    GNNBenchmarkDataset
from torch_geometric.transforms import NormalizeFeatures, Compose
from torch_geometric.utils import homophily, to_scipy_sparse_matrix
sys.path.append("../..")
...
def train(args):
    ...
    def pre_train_oodgat(model, optimizer):
        print("------------------- PreTrain -------------------")
        ...
        prediction = torch.zeros((data.x.shape[0], len(ID_classes))).to(device)
        count = 0
        for epoch in range(args.epochs):
            ...
            if args.w_discrepancy is not None and args.w_discrepancy > 0:
                loss -= torch.pow(a, b * epoch) * args.w_discrepancy * cosine_similarity(att[0].mean(axis=1),
                                                                                         att[1].mean(axis=1))
        ...
        pred_labels[data.train_mask] = data.y[data.train_mask]
        ...
        return pred_labels, pred_entropy

    def selftraining(pseudo_labels, entropy):
        num_class = len(ID_classes)
        num_k = args.num_k
        ...
        for i in sorted_index:
            for j in range(num_class):
                if pseudo_labels[i] == j and count[j] < num_k and not data.train_mask[i]:
                    index.append(i.item())
                    count[j] += 1
        n_train_mask = torch.zeros(data.train_mask.shape, dtype=torch.bool).to(device)
        n_train_mask[data.train_mask] = True
    ...
    pre_labels, pre_entropy = pre_train_oodgat(model, opt)
    new_train_mask = selftraining(pre_labels, pre_entropy)
    pseudo_ID_nodes = torch.sum(torch.logical_and(new_train_mask, ID_mask.to(device)))
    pseudo_ID_rate = pseudo_ID_nodes / torch.sum(new_train_mask)
    model, opt = init_oodgat()
    best_metric, best_t = train_oodgat(model, opt, pre_labels, new_train_mask)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ID_classes', default=[8, 1, 2, 7, 6], type=list)
    parser.add_argument('--splits', default=[3, 10, 5000], type=list)
    ...
    parser.add_argument('--random_seed_data', default=234, type=int)
