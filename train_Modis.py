...
def main():
    ...
    vars(args)['name'] = args.model + '_Modis_' + str(args.num_layers) + '_Layers'
    ...
    for i, seed in enumerate(seed_range):
        ...
        if args.model == 'GCNII':
            ...
            optimizer = th.optim.Adam([{'params': net.parameters(), 'weight_decay': args.weight_decay}],
                                      lr=args.learning_rate)
        train_size = args.train_size if args.train_size else 20
        eta = adj.shape[0] / (adj.sum() / adj.shape[0]) ** args.num_layers
        t = int((train_size * 3 * eta) / (train_size * num_labels))
        ...
        for epoch in range(args.end_epoch):
            val_acc, val_loss = train(net, g, features, labels, labels, train_mask, val_mask, optimizer)
        ...
        n_train_mask = selftraining(prediction, pre_entropy, t, num_labels, train_mask, device)
        ...
        for epoch in range(args.num_epochs_max):
            val_acc, val_loss = train(net, g, features, pre_labels, labels, n_train_mask, val_mask, optimizer)
            print("Epoch {:05d} | Val Loss {:.4f} | Val Acc {:.4f}".format(epoch, val_loss, val_acc))
